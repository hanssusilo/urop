...
Node* Pendulum::GetNodeToExpandWeights() {
  int index = -1;
...
vector<Node*> Pendulum::FindPathIterations(int iterations, int expand_method) {
    ...
    for (int iter = 0; iter < iterations; iter++){
        ...
        /**/
        Node* prev;
        if (expand_method == MIX){
            if (iter < 200){prev = GetNodeToExpandRRT();}
            else{prev = GetNodeToExpandUniform();}
        }
        else if (expand_method == UNIFORM){prev = GetNodeToExpandUniform();}
        else{prev = GetNodeToExpandRRT();}
        ...
        */
        ...
        //Time and work are strictly positive
            ...
            if (((expand_method == MIX) && (iter < 200)) || (expand_method == RRT)){
...
vector<Node*> Pendulum::FindOptimalPathIterations(int iterations, int expand_method){
    vector<Node*> goal_nodes = FindPathIterations(iterations, expand_method);
    vector<Node*> traj;
...
*/        
    ...
    char costFilename [128];
    char obsFilename [128];
    sprintf(costFilename,"cost/path_%s_%f.txt", buffer, traj[traj.size()-1]->cost);
    sprintf(obsFilename,"cost/obs_%s_%f.txt", buffer, traj[traj.size()-1]->cost);
    
    std::ofstream fp_out_obs;
    fp_out_obs.open(obsFilename,std::ios::out);

    for (int i=0; i<obstacles.size(); i++){
        fp_out_obs << obstacles[i]->print();
    }

    std::ofstream fp_out_cost;
    fp_out_cost.open(costFilename,std::ios::out);

    fp_out_cost << b.x << "," << x_epsilon << "," << d.L << endl;
    for (int i=0; i<traj.size(); i++){
        fp_out_cost << traj[i]->print();
    }
...
/*
vector<Node*> Pendulum::RunBenchmark(int datapoints = 50, long iterations = 10000000) {
    cout << "starting path iteration!\n";
    ...
    for (int dpoint = 0; dpoint < datapoints; dpoint++)
    {
        ...
        //BENCHMARKING
        char benchFilename [128];
        sprintf(benchFilename,"benchmark/path_%s_%i.txt", buffer, dpoint);
        std::ofstream fp_out_bench;
        fp_out_bench.open(benchFilename,std::ios::out);
        //BENCHMARKING
        ...
        for (long iter = 0; iter < iterations; iter++){
            if(iter%5000==0){cout<<"iterations:"<<iter<<endl;}
            //BENCHMARKING
            if(iter%1000==0){fp_out_bench << min_cost << "," << iter << endl;}
            //BENCHMARKING
